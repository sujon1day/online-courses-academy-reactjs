  ...
  return (
    <div className="position-fixed">
      ...
      <h5 className="text-success">Enroll Courses: {props.cart.length}</h5>
        <h6>Price: <b>${formatNumber(totalPrice)}</b></h6>
        <h6>Tax + VAT: <b>${formatNumber(tax)}</b></h6>
        <h6>10% OFF Discount <b>${formatNumber(discount)}</b></h6>
        <h4 style={{
          borderTop: "1px solid #000",
          paddingTop: "10px",
          marginTop: "20px"
        }}>Total Amount: <b className="text-success">${formatNumber(grandTotal)}</b></h4>
    </div>
  );
};
